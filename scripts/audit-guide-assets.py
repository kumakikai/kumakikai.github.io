...
generated = {}
errors = []
registry = json.loads((root/'data/uni_guide_images.json').read_text())
preserved = set(json.loads((root/'docs/uni-note-guide-phase2/preservation.json').read_text())['tracked'])

def collect_rendered(route):
...
for source in sorted((root/'content/uni-note-guide').glob('*.md')):
    text=source.read_text()
    route_match=re.search(r'^url:\s*["\']([^"\']+)["\']',text,re.M)
    if not route_match:
        continue
    route=route_match.group(1)
    shots=[]
    for slot_id in re.findall(r'{{<\s*uni-guide-image\s+slot="([^"]+)"\s*>}}',text):
        slot=registry[slot_id]
        if slot['status']=='pending':
            continue
        file=root/'assets'/slot['src']
        if not file.is_file():
            errors.append(f'Missing source {file}');continue
        key=str(file.relative_to(root))
        source_assets[key]={'bytes':file.stat().st_size,'sha256':hashlib.sha256(file.read_bytes()).hexdigest()}
        shots.append({'source':key,'mode':slot.get('mode','crop'),'alt':slot.get('alt'),'caption':slot.get('caption')})
    collect_rendered(route)
    pages.append({'source':str(source.relative_to(root)),'route':route,'app':'uni-note','locale':'ja','images':shots,'watchGuide':False})

unused=[str(p.relative_to(root)) for p in (root/'assets/images/guides').rglob('*') if p.is_file() and str(p.relative_to(root)) not in source_assets and str(p.relative_to(root)) not in preserved]
