...
Apple Pencilで書き始めるところから、PDF、復習、バックアップまでを実画面で案内します。画像は操作部分を抜き出しており、タップすると拡大できます。一般公開版3.5.0の基本操作を案内します。
...
パレットはドラッグで移動できます。ノート上部の「定規」を出すと、指で位置・角度を調整し、近くをApple Pencilでなぞって直線を引けます。「範囲移動」は手書きの線を囲んで移す操作です。写真・PDFは、固定を解除してから個別に動かします。
...
教科カード内の「ノート一覧」から、同じ教科のノートを選べます。ノート画面のボタン配置とパレットは設定でカスタマイズできます。
...
付箋マーカーをパレットから選びます。表示されていない場合は「設定 → 表示設定 → ノート画面 → パレット」で表示する道具を確認します。Apple Pencilで隠したい部分をなぞり、付箋を指でタップすると透過・不透明を切り替えられます。付箋を削除するときは、長押しして表示される削除ボタンをタップします。

{{< guide-image src="images/guides/uni-note/common/sticky-marker.png" alt="Uni:Noteの講義ノート上で答えを隠している黄色い付箋マーカー" mode="crop" >}}

「囲って解答アシスタント」をノートから選びます。見つからない場合は「その他」、または「設定 → 表示設定 → ノート画面 → ナビゲーションバー」で配置を確認します。問題をApple Pencilで囲み、範囲を確認して「解答する」をタップします。
...
問題集は手動でも作成でき、保存後に問題と解答を編集できます。ホームの問題集表示は設定で切り替えられます。
