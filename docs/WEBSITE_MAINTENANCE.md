...
- 初回Phase 2の基準は一般公開3.5.0（ユーザー確認build 6）。2026-09-22の正式対応確認により、「指で描画」は現行機能として既存ガイドに統合する。未確認の次期画面・Premium Plus固有購入などのHOLDは公開本文に加えない。内部安定化の技術説明もガイドへ加えない。
- 2026-09-26の3.7.0サイト更新では、ユーザー指定により3.7.0を現行版として扱う。課金・AI残量・素材操作・録音などの差分と画像判断は [3.7.0差分監査](uni-note-3.7-site-audit.md) に記録する。上記の3.5.0は初回Phase 2制作時の基準であり、現在のサイト本文の版を示さない。
