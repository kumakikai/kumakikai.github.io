---
title: "アプリ一覧"
description: "KUMAKIKAIのアプリ一覧とサポート情報"
---

# アプリ一覧

このサイトでは、KUMAKIKAI のアプリ紹介とサポート情報をまとめています。

## ギガポケ

ギガポケは、povo 2.0から届いたギガコードと外部特典を、
**期限切れになる前に見つけて使うためのiPhone向け管理アプリ**です。

メールからの共有、コピー＆ペースト、手動入力で登録でき、
期限順の一覧、ローカル通知、Small／Medium Widgetから次に使うギガを確認できます。

全機能無料、広告なし、アカウント不要。特典情報はiPhone内に保存し、
共有したメール本文を保存したり、外部サーバーへ送信したりしません。

### リンク

- App Store（審査中）
- [サポート](/notes/2026-09-02-giga-poke/)
- [使い方](/htu/giga-poke/)
- [よくある質問](/faq/giga-poke/)
- [プライバシーポリシー](/privacy/giga-poke/)
- [利用規約](/terms/giga-poke/)

本アプリはpovo 2.0の特典管理を補助する非公式アプリです。KDDI株式会社、povo、auとは提携・協賛・公認その他の関係はありません。

---

## Nocca

*必要なときだけ、静かにつながるための家族通信アプリ（開発中）*

Nocca は、家族との対面会話や長いメッセージが負担になりやすいときに、
**本人主導で必要なことだけ伝えるための iPhone 向けアプリ**です。

ワンタップの意思表示、必要なときだけ開く会話、本人による家族招待と通信休止を中心に、
既読・最終ログイン・入力中表示・GPS・行動ログを持たない設計にしています。

### リンク

- App Store（現在開発中）
- [サポート](/notes/2026-09-06-nocca/)
- [使い方](/htu/nocca/)
- [よくある質問](/faq/nocca/)
- [プライバシーポリシー](/privacy/nocca/)
- [利用規約](/terms/nocca/)

---

## オトミル

*テレビや身近な会話の聞こえを、大きな字幕でサポートするアプリ*

オトミルは、  
テレビや身近な会話の音声を  
**iPhone / iPad の大きな字幕**として表示するアプリです。

テレビの声が聞き取りづらいとき、  
ドライブスルーや外食、1対1の会話で聞き取りづらいときに、  
端末を近くに置いて字幕を確認できます。

### リンク

- [App Store](https://apps.apple.com/jp/app/id6770774613)
- [使い方](/htu/oto-miru/)
- [よくある質問](/faq/oto-miru/)
- [プライバシーポリシー](/privacy/oto-miru/)
- [利用規約](/terms/oto-miru/)

---

## Uni:Note

*紙の大学ノートを、そのまま iPad に持ち込む手書きノートアプリ*

Uni:Note は、  
Apple Pencil での手書きに集中するための  
**iPad専用ノートアプリ**です。

教科やフォルダで整理しながら、
PDF・写真への書き込み、録音、文字起こし、AI学習支援、バックアップまで、
授業中の記録と復習をひとつの流れで扱えます。

### リンク

- [App Store](https://apps.apple.com/jp/app/id6760258084)
- [使い方](/htu/uni-note/)
- [よくある質問](/faq/uni-note/)
- [プライバシーポリシー](/privacy/uni-note/)

---

## Uni:Note Pocket

*Uni:Note のバックアップを iPhone で見返すための companion app*

Uni:Note Pocket は、  
Uni:Note で作成したバックアップを取り込み、  
**iPhone で安全に見返すための閲覧専用アプリ**です。

ノート一覧、ページ閲覧、暗記レイヤーの表示切替に絞り、  
書く・編集する責務を持ち込まない最小構成で設計しています。

### リンク

- [App Store](https://apps.apple.com/jp/app/id6761449487)
- [使い方](/htu/uni-note-pocket/)
- [よくある質問](/faq/uni-note-pocket/)
- [プライバシーポリシー](/privacy/uni-note-pocket/)

---

## ギャンカレ

*日々の収支を、感情や感覚ではなく「事実」として残すための記録アプリ*

ギャンカレ は、  
日々の収支や数字を、あとから正確に振り返るための  
**シンプルな収支記録アプリ**です。

感覚や印象ではなく、  
実際に何が起きていたかを  
静かに確認することに特化しています。

### リンク

- [App Store](https://apps.apple.com/jp/app/id6757731648)
- Google Play（現在開発中）
- [使い方](/htu/balance-calendar/)
- [よくある質問](/faq/balance-calendar/)
- [プライバシーポリシー](/privacy/balance-calendar/)

---

## すわなび

*吸った回数と、吸わずに済んだ回数を、できるだけ手早く残すための記録アプリ*

すわなび は、  
「吸った」と「我慢した」を  
**シンプルに記録して振り返るためのアプリ**です。

禁煙の補助としても、  
今の喫煙状況を数字で把握するための記録ツールとしても使えます。

ワンタップ中心で記録でき、  
日別 / 月別 / 年別の一覧や、週 / 月 / 3か月のグラフで変化を確認できます。  
`1箱の本数` と `1箱の金額` を設定すれば、金額換算もできます。

### リンク

- [App Store](https://apps.apple.com/jp/app/id6760842941)
- [使い方](/htu/smokeless/)
- [よくある質問](/faq/smokeless/)
- [プライバシーポリシー](/privacy/smokeless/)

---

## SIGNAL

*個人発信を中心にしたニュースフィードアプリ*

SIGNAL は、  
note、Qiita、Zenn、はてなブックマークなどの  
**個人発信を主軸に構成されたニュースアプリ**です。

広告なし・通知なし・ログイン不要。  
学習は端末内で完結します。

### リンク

- [App Store](https://apps.apple.com/jp/app/id6759493613)
- [使い方](/htu/signal/)
- [よくある質問](/faq/signal/)
- [プライバシーポリシー](/privacy/signal/)

---

## お問い合わせ

- [kumakikai.apps@gmail.com](mailto:kumakikai.apps@gmail.com)
