---
title: "FAQ (Smokeless)"
description: "Frequently asked questions about Smokeless."
lastmod: 2026-09-08
---

{{< guide-anchor "basics" >}}

## Introduction {#introduction}

### What kind of app is Smokeless? {#what-kind-of-app-is-smokeless}

It is an app for quickly recording `Smoked` and `Avoided` and reviewing counts and money values later. It can be used both as support for quitting smoking and as a simple log tool.

### What does `Avoided` record? {#what-does-avoided-record}

It records a time when you wanted to smoke but did not. It does not automatically count smoke-free time.

### Is this a medical app? {#is-this-a-medical-app}

No. It does not provide medical treatment or medical services.

### Which devices and OS versions are supported? {#environment}

Requires an iPhone with iOS 14 or later.

{{< guide-anchor "recording-and-review" >}}

## Recording and review {#questions-8}

### Is there a first-time tutorial? {#is-there-a-first-time-tutorial}

Yes. It appears on first launch. See [the first tutorial and pack settings]({{< relref "/htu/smokeless/" >}}#1-check-the-first-tutorial-and-pack-settings) for the initial setup.

### What can I check? {#what-can-i-check}

You can review counts, money values and trends. See [daily, monthly and yearly review in Calendar]({{< relref "/htu/smokeless/" >}}#4-review-by-day-month-and-year-in-calendar) for the calendar controls.

### How is the money amount calculated? {#how-is-the-money-amount-calculated}

The amount per cigarette is calculated from `Cigarettes per pack` and `Price per pack`, and `Smoked` and `Avoided` are converted into money values.

### Can I record past dates too? {#can-i-record-past-dates-too}

No. At this time, recording is supported only for the current day.

{{< guide-anchor "features" >}}

## Features {#questions-14}

### Is there a home widget? {#is-there-a-home-widget}

Yes. See the [widget instructions]({{< relref "/htu/smokeless/" >}}#6-use-the-home-widget).

### Can I add multiple brands? {#can-i-add-multiple-brands}

One brand is available initially. Purchasing the Extra brands pack allows up to four brands in total.

### Is there an `Over-smoking alert`? {#is-there-an-over-smoking-alert}

Yes. Configure it in Settings → General. It asks for confirmation when a smoked entry on iPhone would exceed your selected daily count.

### Can I change the language or currency? {#can-i-change-the-language-or-currency}

Yes. Open Settings → General.

{{< guide-anchor "data-and-storage" >}}

## Data {#data}

### Where is the data stored? {#where-is-the-data-stored}

Records and settings are stored on the device.

### Are backup or cloud sync available? {#are-backup-or-cloud-sync-available}

Cloud sync and file-based backups are not available. Apple Watch communication is separate from cloud backup.

### Do I need to register an account? {#do-i-need-to-register-an-account}

No. You can use it without login or membership.

## Purchases {#purchases}

### Where are ads shown? {#where-are-ads-shown}

A banner appears on Log. Starting one week after the in-app ad notice, a full-screen ad may appear at most once per week when opening Calendar or Charts.

### Can I remove ads? {#can-i-remove-ads}

Yes. You can buy a one-time purchase for `Remove ads`. Restore purchase is also supported.

## Help {#help}
### Can I restore a record deleted by mistake? {#can-i-restore-a-record-deleted-by-mistake}

If it was just deleted, you can restore it with `Undo`.

### Why are Apple Watch records not appearing on iPhone? {#why-are-apple-watch-records-not-appearing-on-iphone}

The Apple Watch update is under review. See the [Apple Watch guide]({{< relref "/htu/smokeless/" >}}#apple-watch) for requirements, offline storage and transfer after reconnection.

### What should I do if something goes wrong? {#troubleshooting}

{{< document-copy "troubleshooting" >}}

### How can I contact support? {#contact}

{{< document-contact >}}
