---
title: "使い方 (Uni:Note)"
description: "Uni:Noteの公式ユーザーガイド。ノート作成、Apple Pencil、指での描画、写真・PDF、録音、AI、問題集、バックアップ、便利な使い方を目的から探せます。"
seo_title: "使い方 (Uni:Note) | Uni:Note 使い方"
type: "uni-guide"
url: "/htu/uni-note/"
lastmod: "2026-09-22"
guide_category: ""
guide_kind: "top"
---

現行版の基本操作を案内します。初めてのノート作成から、資料・録音・AIを組み合わせた復習まで、目的に合わせて説明を選べます。

対応端末はiPad、対応OSはiPadOS 17.0以降です。Apple Pencilで従来どおり手書きでき、「指で描画」をONにすると1本指でも書けます。文字起こしには対応するiPadOS 26以降の環境が必要です。

{{< uni-guide-search >}}

## 機能から探す

三角ボタンで内容を展開できます。大項目名を選ぶとガイドの先頭へ、展開した項目を選ぶと該当する説明へ進みます。

{{< guide-anchor "1-ホームで教科とフォルダを整理する" "2-最初のノートを開く" "create" >}}

{{< uni-guide-category category="start" >}}

{{< uni-guide-category category="library" >}}

{{< guide-anchor "6-ノート一覧-と-ページ一覧-を使う" "find" >}}

{{< uni-guide-category category="notebooks" >}}

{{< guide-anchor "3-apple-pencilで書く" "4-2本指でズームする" "5-ツールを切り替える" "9-定規-を使う" "write" >}}

{{< uni-guide-category category="writing" >}}

{{< guide-anchor "7-その他-メニューを使う" "8-写真とpdfを使う" "pdf" >}}

{{< uni-guide-category category="materials" >}}

{{< guide-anchor "10-録音文字起こしai要約を使う" "recording" >}}

{{< uni-guide-category category="recording" >}}

{{< guide-anchor "11-暗記を使う" "review" >}}

{{< uni-guide-category category="study" >}}

{{< guide-anchor "12-囲って解答アシスタント-を使う" >}}

{{< uni-guide-category category="ai" >}}

{{< guide-anchor "13-問題集を作る" >}}

{{< uni-guide-category category="problem-sets" >}}

{{< uni-guide-category category="customize" >}}

{{< guide-anchor "14-保護ゴミ箱" "protect" >}}

{{< uni-guide-category category="data" >}}

{{< guide-anchor "15-設定バックアップ" "backup" >}}

{{< uni-guide-category category="backup" >}}

{{< uni-guide-category category="plans" >}}

{{< guide-anchor "split-view--slide-over--2ペインで使う" "windows" >}}

{{< uni-guide-category category="ipados" >}}

{{< uni-guide-category category="workflows" >}}
