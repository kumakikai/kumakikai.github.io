---
title: "應用程式"
description: "KUMAKIKAI 應用程式列表與支援資訊。"
---

# 應用程式

此網站提供 KUMAKIKAI 應用程式介紹與支援資訊。

## Uni:Note

*把紙本大學筆記本的感覺帶進 iPad 的手寫筆記 App*

Uni:Note 是一款專注於 Apple Pencil 手寫體驗的  
**iPad 專用筆記 App**。

你可以依科目整理、依筆記本區分，  
盡量保留接近紙本筆記本的使用感。

### 連結

- [App Store](https://apps.apple.com/jp/app/id6760258084)
- [使用方式]({{< relref "/htu/uni-note/" >}})
- [常見問題]({{< relref "/faq/uni-note/" >}})
- [隱私權政策]({{< relref "/privacy/uni-note/" >}})

## Uni:Note Pocket

*用來在 iPhone 上重新查看 Uni:Note 備份的 companion app*

Uni:Note Pocket 會匯入 Uni:Note 建立的備份，  
讓你可以**在 iPhone 上安全地重新查看內容**。

它以唯讀 companion app 為前提，  
聚焦在筆記列表、頁面瀏覽與記憶遮罩顯示切換，不加入書寫與編輯功能。

### 狀態

- 發布準備中

### 連結

- App Store（準備中）
- [更新紀錄]({{< relref "/updates/uni-note-pocket/" >}})
- [使用方式]({{< relref "/htu/uni-note-pocket/" >}})
- [常見問題]({{< relref "/faq/uni-note-pocket/" >}})
- [隱私權政策]({{< relref "/privacy/uni-note-pocket/" >}})

## Balance Calendar

*把每日收支當作事實而不是感覺留下來的記錄 App*

Balance Calendar 是一款用來精確回看每日收支的  
**簡單記錄 App**。

它重點不在印象，而是在安靜地確認實際發生了什麼事。

### 連結

- [App Store](https://apps.apple.com/jp/app/id6757731648)
- 支援頁面目前僅提供日文

## Smokeless

*快速記下抽了幾次，以及成功忍住幾次的記錄 App*

Smokeless 讓你可以簡單記錄「抽了」與「忍住了」。
你可以用日、月、年列表，以及週、月、3 個月圖表回看變化，也能依照每包支數與金額估算花費。

### 連結

- [App Store](https://apps.apple.com/jp/app/id6760842941)
- [使用方式]({{< relref "/htu/smokeless/" >}})
- [常見問題]({{< relref "/faq/smokeless/" >}})
- [隱私權政策]({{< relref "/privacy/smokeless/" >}})

## SIGNAL

*以個人發佈內容為核心的新聞資訊 App*

SIGNAL 以 note、Qiita、Zenn、Hatena Bookmark 等來源為主軸。
沒有廣告、沒有通知、不需登入，學習流程都在裝置內完成。

### 連結

- [App Store](https://apps.apple.com/jp/app/id6759493613)
- 支援頁面目前僅提供日文

## 聯絡方式

- [kumakikai.apps@gmail.com](mailto:kumakikai.apps@gmail.com)
